...
const ImageUploadTab = ({ onClose, onUploadSuccess }) => {
    ...
    return (
        <div className="space-y-5">
            ...
            >
                <input ref={inputRef} type="file" accept="image/*" className="hidden" onChange={(e) => setFile(e.target.files[0])} disabled={status === 'loading'} />
    ...
    return (
        <div className="space-y-4">
            <div className="space-y-1.5">
                ...
                <input className={inputClass} placeholder="e.g. Superpharm" value={form.merchant_name} onChange={(e) => handleChange('merchant_name', e.target.value)} />
            </div>
            ...
            <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-3 gap-2">
                    {CATEGORIES.map(c => (
                        <button key={c.id} type="button" onClick={() => handleChange('category', c.id)} className={`flex items-center justify-center gap-2 py-2 rounded-xl border text-xs font-medium transition-all ${form.category === c.id ? 'border-teal bg-teal/5 text-teal shadow-sm' : 'border-slate-100 text-slate-500 hover:bg-slate-50'}`}>
                            <span>{c.icon}</span>{c.label}
                        </button>
                    ))}
                </div>
            </div>
            <button onClick={handleSave} disabled={!form.merchant_name || !form.receipt_date || !form.total_amount || loading} className="w-full py-3 text-sm font-bold rounded-xl text-white bg-teal disabled:opacity-40 transition-all shadow-md active:scale-[0.98]">
