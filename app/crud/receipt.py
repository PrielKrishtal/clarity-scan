...
async def create_manual_receipt(
    db: AsyncSession, receipt: schemas.ReceiptCreate, user_id: int
):
    new_receipt = Receipt(
        **receipt.model_dump(),
        user_id=user_id,
...
# 4. UPDATE
async def update_user_receipt(
    db: AsyncSession, db_receipt: Receipt, update_data: schemas.ReceiptUpdate
):
